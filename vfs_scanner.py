    ...
    async def scan_country(self, country_code: str, country_name: str) -> Dict:
        ...
        try:
            ...
            for selector in config['selectors']['no_appointment']:
                try:
                    ...
                    if count > 0:
                        no_appt_found = True
                        print(
                            f"   ✓ Found 'no appointment' message: {selector}")
                        break
                except Exception:
                    continue
                ...
                for selector in config['selectors']['appointment_slots']:
                    try:
                        ...
                        if len(slots) > 0:
                            ...
                            print(
                                f"   ✓ Found {len(slots)} slots with: {selector}"
                            )

                            # Extract dates from slots
                            for i, slot in enumerate(
                                    slots[:10]):  # Max 10 slots
                                try:
            ...
            return {
                ...
                'available_slots':
                available_slots if has_appointment else None,
                'message': message,
